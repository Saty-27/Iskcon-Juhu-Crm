...
export default function EventDonation() {
  ...
  const handleDonateClick = (amount?: number) => {
    const donationAmount = amount || parseFloat(selectedPrice) || 0;
    if (donationAmount > 0) {
      window.location.href = `/donate/payment?eventId=${event?.id}&amount=${donationAmount}`;
    } else {
  ...
  const currentBankDetail = bankDetails.find(bd => bd.isActive) || bankDetails[0];
          ...
          <div style={{
            ...
            {eventCards.map((card) => (
              <div key={card.id} style={{
                ...
                {/* Donate Button */}
                <Button 
                  onClick={() => handleDonateClick(card.amount)}
                  style={{
                    backgroundColor: '#faa817',
                    color: '#fff',
                    border: 'none',
